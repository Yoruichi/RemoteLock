...
public class LockLockRedisConfig {
    @Bean
    public GenericObjectPool<StatefulRedisConnection<String, String>> lettucePool(@Value("${spring.redis.host.locklock:localhost}") String host,
        ...
        RedisURI.Builder redisUriBuilder = RedisURI.builder().withHost(host).withPort(port).withDatabase(db);
    ...
    @Bean
    public AsyncPool<StatefulRedisConnection<String, String>> lettuceAsyncPool(@Value("${spring.redis.host.locklock:localhost}") String host,
            @Value("${spring.redis.port.locklock:6379}") int port,
            @Value("${spring.redis.database.locklock:0}") int db,
            @Value("${spring.redis.password.locklock:}") String password,
            @Value("${spring.redis.pool.max-active.locklock:64}") int maxActive,
        ...
        AsyncPool<StatefulRedisConnection<String, String>> pool = AsyncConnectionPoolSupport
                .createBoundedObjectPool(() -> redisClient.connectAsync(StringCodec.UTF8, redisUriBuilder.build()),
                        BoundedPoolConfig.builder().maxIdle(maxIdle).maxTotal(maxActive).minIdle(minIdle).build());
